...
for i in xrange(16,18):
  ...
  count = count+10
  for j in xrange(1,11):
    child.sendline("inter eth " + str(i) +"/1."+str(j))
    child.expect("#")
    child.sendline("ip add 55."+str(i)+"."+str(j)+".2/24")
    child.expect("#")
    child.sendline("mac security profile test2")
    child.expect("#")
    child.sendline("mac security proxy patch eth "+str(i)+"/1."+str(j+10))
    child.expect("#")
    child.sendline("inter eth "+str(i)+"/1."+str(j+10))
    child.expect("#")
    child.sendline("vlan id "+str(j+count))
    child.expect("#")
    child.sendline("inter vx1")
