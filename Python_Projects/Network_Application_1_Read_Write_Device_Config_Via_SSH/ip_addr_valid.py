# Make sure all the IP addresses specified in the IP address file are valid or not
import sys

# checking octets for each IP address in the list
def ip_addr_valid(iplist):
    for ip in iplist:
        ip.rstrip("\n") # remove (trailing) next line character from each ip in the for loop
        octet_list = ip.split(".") # Convert ip address into list of all octets
